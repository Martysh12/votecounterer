...
def nextcordise(image, filename, desc, spoiler=True):
    ...
    return nextcord.File(
        fp=output, filename=filename, description=desc, spoiler=spoiler
    )
...
def draw_text_aligned(
    draw, text, bbox, alignment, anchor, offset, font=None, fill="white"
):
    ...
    draw.text(
        (
            bbox[0]
            + bbox_wh[0] * alignment[0]
            - (text_bbox[2] * anchor[0])
            + offset[0],
            bbox[1]
            + bbox_wh[1] * alignment[1]
            - (text_bbox[3] * anchor[1])
            + offset[1],
        ),
        text,
        font=font,
...
def draw_text_segmented(draw, pos, text_segments):
    ...
    for seg in text_segments:
        ...
        if "bg" in seg:
            draw.rectangle(
                xywh_to_bbox(final_bbox[0], final_bbox[1], bbox[2], bbox[3]),
                fill=seg["bg"],
            )

        if "draw" not in seg or seg["draw"]:
            draw.text(
                (pos[0] + final_bbox[2], pos[1]),
                text,
                font=seg["font"],
...
def stats_graph(stats):
    ...
    draw_text_segmented(
        ...
        TITLE_LOCATION,
        [
            {"text": "Stats, as of ", "font": FONT_TITLE},
            {
    ...
    pieslice_bbox = xywh_to_bbox(
        ...
        TITLE_LOCATION[1]
        + FONT_TITLE.getbbox("Stats, as of ")[3]
        + GRAPH_MARGIN,
        HEIGHT
        - (
            TITLE_LOCATION[1]
    ...
    # Draw comment pie
    draw.pieslice(pieslice_bbox, -angles[0], 0, fill=PIE_COLOURS["vote"])

    draw.pieslice(pieslice_bbox, 0, angles[1], fill=PIE_COLOURS["nonvote"])
    ...
    draw.ellipse(pieslice_bbox, width=4)
    ...
    initial_pos = (
        ...
        pieslice_bbox[3]
        - (legend_colour_bbox[3] * (1.5 * 4))
        + (legend_colour_bbox[3] * 0.5),
    )
    ...
    bbox = draw_text_segmented(
        ...
        [
            ...
            {"text": " ", "font": FONT_DATA, "draw": False},
            {"text": "Valid vote comments", "font": FONT_NORMAL},
        ],
    )
    ...
    draw_text_segmented(
        draw,
        (initial_pos[0], initial_pos[1] + bbox[3] * 1.5),
        [
            {
                "text": "__",
                "font": FONT_DATA,
            ...
            {"text": " ", "font": FONT_DATA, "draw": False},
            {"text": "Non-vote comments", "font": FONT_NORMAL},
        ],
    )
    ...
    draw_text_segmented(
        draw,
        (initial_pos[0], initial_pos[1] + bbox[3] * 1.5 * 2),
        [
            {
                "text": "__",
                "font": FONT_DATA,
            ...
            {"text": " ", "font": FONT_DATA, "draw": False},
            {"text": "Duplicate votes", "font": FONT_NORMAL},
        ],
    )
    ...
    draw_text_segmented(
        draw,
        (initial_pos[0], initial_pos[1] + bbox[3] * 1.5 * 3),
        [
            {
                "text": "__",
                "font": FONT_DATA,
            ...
            {"text": " ", "font": FONT_DATA, "draw": False},
            {"text": "Late votes", "font": FONT_NORMAL},
        ],
    )
    ...
    bbox = draw_text_segmented(
        ...
        initial_pos,
        [
            {"text": "There are ", "font": FONT_NORMAL},
            {"text": stats["total_comments"], "font": FONT_DATA_BOLD},
            {"text": " comments in total.", "font": FONT_NORMAL},
        ],
    )
    ...
    draw_text_segmented(
        draw,
        (initial_pos[0], initial_pos[1] + bbox[3] * 1.5 * line_number),
        [
            {
            ...
            {"text": " (about ", "font": FONT_NORMAL},
            {
                "text": str(
                    round(
                        stats["vote_comments"] / stats["total_comments"] * 100,
                        1,
                    )
                )
                + "%",
            ...
            {"text": ") of them are ", "font": FONT_NORMAL},
            {"text": "votes", "font": FONT_NORMAL_BOLD},
    ...
    draw_text_segmented(
        ...
        [
            ...
            {"text": ") of them are ", "font": FONT_NORMAL},
            {"text": "normal comments", "font": FONT_NORMAL_BOLD},
            {"text": ".", "font": FONT_NORMAL},
        ],
    )
    ...
    if stats["late_votes"] > 0:
        draw_text_segmented(
            ...
            [
                ...
                {"text": ") of them are ", "font": FONT_NORMAL},
                {"text": "late votes", "font": FONT_NORMAL_BOLD},
                {"text": ".", "font": FONT_NORMAL},
            ],
        )
    ...
    draw_text_segmented(
        ...
        [
            ...
            {"text": ") of them are ", "font": FONT_NORMAL},
            {"text": "duplicated votes", "font": FONT_NORMAL_BOLD},
            {"text": ".", "font": FONT_NORMAL},
        ],
    )
    ...
    draw_text_segmented(
        ...
        [
            ...
            {"text": " commenters have submitted ", "font": FONT_NORMAL},
            {"text": "duplicated votes", "font": FONT_NORMAL_BOLD},
            {"text": ".", "font": FONT_NORMAL},
        ],
    )
    ...
    draw_text_segmented(
        ...
        (initial_pos[0], initial_pos[1] + bbox[3] * 1.5 * line_number),
        [
            {"text": "The most prolific one is ", "font": FONT_NORMAL},
            {
    ...
    draw_text_segmented(
        ...
        (initial_pos[0], initial_pos[1] + bbox[3] * 1.5 * line_number),
        [
            {"text": "who submitted ", "font": FONT_NORMAL},
            {
                "text": stats["most_prolific_duplicator_votes"],
                "font": FONT_DATA_BOLD,
                "fill": PIE_COLOURS["duplicate"],
            },
            {"text": " of them.", "font": FONT_NORMAL},
        ],
    )
...
def votes_graph(stats, sort=False):
    ...
    draw_text_segmented(
        ...
        TITLE_LOCATION,
        [
            {"text": "Votes, as of ", "font": FONT_TITLE},
            {
                "text": stats["counted_at"].strftime(DATETIME_FORMAT),
                "font": FONT_TITLE_BOLD,
            },
    ...
    votebox_bbox = (
        WIDTH / 2,
        TITLE_LOCATION[1]
        + FONT_TITLE.getbbox("Votes, as of ")[3]
        + GRAPH_MARGIN,
        WIDTH - GRAPH_MARGIN,
        HEIGHT - GRAPH_MARGIN,
    )
    ...
    for i, (letter, votes) in enumerate(vote_items):
        ...
        draw.rectangle(bar_bbox, fill=VOTE_COLOURS[i])
        ...
        if percentage > 0.5:
            ...
            bbox = draw_text_aligned(
                ...
                (
                    0,
                    -FONT_DATA_BOLD.getbbox(f"[{letter}]")[3] * 0.25 - bbox[3],
                ),
                font=FONT_DATA_BOLD,
                fill="white",
            )
    ...
    draw.line(
        (votebox_bbox[0], votebox_bbox[1], votebox_bbox[0], votebox_bbox[3]),
        width=2,
    )
    draw.line(
        (votebox_bbox[0], votebox_bbox[3], votebox_bbox[2], votebox_bbox[3]),
        width=2,
    )
    draw.line(
        (votebox_bbox[2], votebox_bbox[3], votebox_bbox[2], votebox_bbox[1]),
        width=2,
    )
    ...
    for letter, votes in stats["votes"].items():
        bbox = draw_text_segmented(
            ...
            [
                ...
                {"text": " - ", "font": FONT_DATA_BIGGER},
                {"text": str(votes), "font": FONT_DATA_BIGGER_BOLD},
                {"text": " (", "font": FONT_DATA_BIGGER},
                {
